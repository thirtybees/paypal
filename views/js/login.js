(function () {
  function initPayPalLogin() {
    if (typeof window.paypal === 'undefined') {
      setTimeout(initPayPalLogin, 100);
      return;
    }
    ...
    window.paypal.loginUtils = {
      ...
      getCenteredPosition: function (elWidth, elHeight) {
        var viewportHeight = window.innerHeight || document.documentElement.clientHeight,
          ...
          viewportLeft = window.screenX || window.screenLeft, viewportTop = window.screenY || window.screenTop;
        return {
      ...
      styleFromString: function (css) {
        var doc = document, head = doc.head || doc.getElementsByTagName("head")[0], rand = this.createRandomNumber(),
          id = "paypal-css-" + rand, style = doc.createElement("style");
        style.type = "text/css";
        if (style.styleSheet) {
      ...
      loadAsync: function (url, callback) {
        var doc = document, head = doc.head || doc.getElementsByTagName("head")[0], rand = this.createRandomNumber(),
          fullUrl = url + "?" + "b=" + rand, id = "loader_" + rand, isJS = (/\.js$/.test(url)) ? true : false,
          newEl = (isJS) ? doc.createElement("script") : doc.createElement("link");
        newEl.id = id;
        if (isJS) {
          newEl.src = fullUrl;
    ...
    window.paypal.login = {
      ...
      render: function (params) {
        ...
        var popupWidth = 400, popupHeight = 560,
          authEndPoint = (params.authend) ? "https://www.sandbox.paypal.com/webapps/auth/protocol/openidconnect/v1/authorize" : "https://www.paypal.com/webapps/auth/protocol/openidconnect/v1/authorize",
          appId = params.appid || null, returnUrl = params.returnurl || null, scopes = params.scopes || "basic",
          container = (params.containerid) ? document.getElementById(params.containerid) : (params.containerelement) ? params.containerelement : thisScriptEl,
          translationText = window.paypal.loginUtils.getTranslation(params),
          theme = (params.theme && params.theme.toLowerCase() === "neutral") ? "" : " PPBlue", query = {
            client_id: appId,
            response_type: "code",
            scope: scopes,
